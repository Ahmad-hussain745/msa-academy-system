...
import { toggleStudentFeeActive, deleteStudentFee } from "./actions";

function fmt(n) {
  return "Rs. " + Math.round(Number(n) || 0).toLocaleString("en-US");
}

export default function StudentFeeRow({ row, isCurrent }) {
  ...
  const handleToggle = () => {
    ...
    startTransition(async () => {
      const res = await toggleStudentFeeActive(row.id, !row.active);
  ...
  const handleDelete = () => {
    if (!confirm(`Delete this override (Rs. ${row.monthly_fee}, effective ${row.effective_from})?`)) return;
    setError("");
    startTransition(async () => {
      const res = await deleteStudentFee(row.id);
      if (res?.error) { setError(res.error); return; }
      router.refresh();
    });
  };
  ...
  return (
    <>
      <tr className="border-t border-slate-100">
        ...
        <td className="px-4 py-3 text-right font-mono">{fmt(row.monthly_fee)}</td>
