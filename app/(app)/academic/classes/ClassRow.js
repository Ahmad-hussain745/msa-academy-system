...
import { updateClass, toggleClassStatus, deleteClass } from "./actions";

export default function ClassRow({ klass }) {
  const [editing, setEditing] = useState(false);
  const [name, setName] = useState(klass.name);
  const [sortOrder, setSortOrder] = useState(klass.sort_order);
  ...
  const handleToggle = () => {
    setError("");
    startTransition(async () => {
      const res = await toggleClassStatus(klass.id, klass.status !== "active");
      if (res?.error) { setError(res.error); return; }
      router.refresh();
    });
  };
  ...
  const handleDelete = () => {
    if (!confirm(`Delete "${klass.name}"? This can't be undone.`)) return;
    setError("");
    startTransition(async () => {
      const res = await deleteClass(klass.id);
      if (res?.error) { setError(res.error); return; }
      router.refresh();
    });
  };
      ...
      <tr className="border-t border-slate-100">
        <td className="px-4 py-2" colSpan={3}>
          <form onSubmit={handleSave} className="flex items-center gap-2">
            ...
            <input type="number" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="border border-slate-300 rounded-lg px-2 py-1 text-sm w-20" />
  ...
  return (
    <>
      <tr className="border-t border-slate-100">
        <td className="px-4 py-3 font-medium text-ink">{klass.name}</td>
        <td className="px-4 py-3 text-slate-600">{klass.sort_order}</td>
        <td className="px-4 py-3">
          <span className={`text-xs px-2 py-0.5 rounded-full ${klass.status === "active" ? "bg-green-50 text-sage" : "bg-slate-100 text-slate-500"}`}>
            {klass.status}
          </span>
        </td>
